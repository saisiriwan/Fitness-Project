...
export function SignUpPage({ onSignUp, onBackToLogin }: SignUpPageProps) {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const lastName =
        nameParts.length > 1 ? nameParts.slice(1).join(" ") : "Trainee";
      ...
      if (error.response?.status === 409) {
        toast.error("อีเมลนี้ถูกใช้งานแล้ว กรุณาใช้อีเมลอื่นหรือเข้าสู่ระบบ");
      } else {
        toast.error(
          error.response?.data?.error ||
            "การสมัครสมาชิกไม่สำเร็จ โปรดลองอีกครั้ง",
        );
      }
    }
  };
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-4">
          ...
          <div className="text-center">
            ...
            <CardDescription>
              สร้างบัญชีเพื่อเริ่มต้นการฝึกของคุณ
            </CardDescription>
          </div>
        </CardHeader>
